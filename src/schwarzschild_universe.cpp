...
const std::vector<Ray2D> *SchwarzschildUniverse::get_rays() const
{
    return &_rays;
}
...
const std::vector<std::tuple<float, Ray2D>> *SchwarzschildUniverse::get_ray_trails() const
{
    return &_ray_trails;
}
...
PolarTransform2D SchwarzschildUniverse::geodesic(const PolarTransform2D &k, const double E)
{
	double dr   = k.d_r;
	double dphi = k.d_phi;
...
{
    const PolarTransform2D k1 = geodesic(ray.transform, ray.E);

    const PolarTransform2D y2 = ray.transform + k1 * (dlambda * 0.5);
    const PolarTransform2D k2 = geodesic(y2, ray.E);

    const PolarTransform2D y3 = ray.transform + k2 * (dlambda * 0.5);
    const PolarTransform2D k3 = geodesic(y3, ray.E);

    const PolarTransform2D y4 = ray.transform + k3 * dlambda;
    const PolarTransform2D k4 = geodesic(y4, ray.E);
