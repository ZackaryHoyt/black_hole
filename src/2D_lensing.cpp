...
struct Ray
{
	...
	Ray(glm::vec2 pos, glm::vec2 dir) : x(pos.x), y(pos.y), r(sqrt(pos.x * pos.x + pos.y * pos.y)), phi(atan2(pos.y, pos.x)), dr(dir.x), dphi(dir.y)
	{
	...
	void step(double dλ, double rs)
	{
		...
		if(r <= rs)
		{
			return; // stop if inside the event horizon
		}
		rk4Step(*this, dλ, rs);
